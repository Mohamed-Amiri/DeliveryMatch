...
<<<<<<< HEAD
        try {
            ...
            logger.debug("Processing JWT for user: {} on path: {}", userEmail, request.getServletPath());

            if (userEmail != null && SecurityContextHolder.getContext().getAuthentication() == null) {
                UserDetails userDetails = this.userDetailsService.loadUserByUsername(userEmail);
                logger.debug("User details loaded: {}, enabled: {}", userDetails.getUsername(), userDetails.isEnabled());
                ...
                if (jwtService.isTokenValid(jwt, userDetails)) {
                    ...
                    logger.debug("Authentication successful for user: {} on path: {}", userEmail, request.getServletPath());
                    logger.debug("User authorities: {}", userDetails.getAuthorities());
                } else {
                    logger.debug("Invalid JWT token for user: {} on path: {}", userEmail, request.getServletPath());
                }
            } else if (userEmail == null) {
                logger.debug("Could not extract username from JWT token");
            } else {
                logger.debug("Authentication already exists for user: {}", userEmail);
            }
        } catch (Exception e) {
